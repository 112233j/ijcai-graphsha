...
for seed in range(0,5):
    ...
    dataset = get_dataset(args.dataset, path, split_type='public')
    data = dataset[0]
    ...
    final_bACC.append(test_bacc*100)
    final_f1.append(test_f1*100)
with open('result.txt', 'a') as f:
    f.write(f'dataset:{args.dataset}, net:{args.net}, ratio:{args.imb_ratio}, bacc: {np.mean(final_bACC)} $\pm$ {np.std(final_bACC)}, f1: {np.mean(final_f1)} $\pm$ {np.std(final_f1)} \n')
